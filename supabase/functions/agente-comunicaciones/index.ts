...
serve(async (req) => {
  ...
  try {
    const { prompt, publicaciones, videosIA } = await req.json();
...
${parsedPrompt.caption}`;
      ...
      userMsg = `Generá un prompt detallado en inglés para crear un video con IA (Runway Gen-3 / Kling) que muestre visualmente el siguiente ejercicio de simulación médica.
